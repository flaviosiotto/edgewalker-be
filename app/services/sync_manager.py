"""
Sync Manager - Generic background synchronization orchestrator.

This module provides a lightweight, extensible framework for background
synchronization tasks. It runs per-connection and can handle multiple
sync types (symbols, contracts, etc.) in the future.

Key features:
- Automatic startup sync (initial load or incremental)
- Configurable polling interval per connection
- Incremental change detection via content hashing
- Non-blocking async execution
- Graceful shutdown
"""
from __future__ import annotations

import asyncio
import logging
from abc import ABC, abstractmethod
from concurrent.futures import ThreadPoolExecutor
from datetime import datetime, timedelta
from enum import Enum
from functools import partial
from typing import Any, Callable

from sqlmodel import Session, select

from app.core.config import settings
from app.db.database import get_session_context
from app.models.connection import Connection

logger = logging.getLogger(__name__)


class SyncType(str, Enum):
    """Types of sync operations."""
    SYMBOLS = "symbols"
    # Future: CONTRACTS, INSTRUMENTS, etc.


class SyncStatus(str, Enum):
    """Status of a sync operation."""
    SUCCESS = "success"
    CONNECTED = "connected"
    NOT_CONNECTED = "not_connected"
    SKIPPED = "skipped"
    NO_CHANGES = "no_changes"
    ERROR = "error"


class SyncResult:
    """Result of a sync operation."""

    def __init__(
        self,
        status: SyncStatus,
        sync_type: SyncType,
        connection_name: str,
        items_fetched: int = 0,
        items_added: int = 0,
        items_updated: int = 0,
        items_removed: int = 0,
        is_incremental: bool = False,
        content_hash: str | None = None,
        message: str | None = None,
        error: str | None = None,
        duration_seconds: float = 0,
    ):
        self.status = status
        self.success = status in (SyncStatus.SUCCESS, SyncStatus.NO_CHANGES, SyncStatus.CONNECTED)
        self.sync_type = sync_type
        self.connection_name = connection_name
        self.items_fetched = items_fetched
        self.items_added = items_added
        self.items_updated = items_updated
        self.items_removed = items_removed
        self.is_incremental = is_incremental
        self.content_hash = content_hash
        self.message = message
        self.error = error
        self.duration_seconds = duration_seconds

    def __repr__(self) -> str:
        if self.status == SyncStatus.SUCCESS:
            return (
                f"SyncResult({self.sync_type.value}:{self.connection_name} "
                f"+{self.items_added} ~{self.items_updated} -{self.items_removed} "
                f"in {self.duration_seconds:.1f}s)"
            )
        elif self.status == SyncStatus.CONNECTED:
            return f"SyncResult({self.sync_type.value}:{self.connection_name} CONNECTED)"
        elif self.status == SyncStatus.NOT_CONNECTED:
            return f"SyncResult({self.sync_type.value}:{self.connection_name} NOT_CONNECTED: {self.message})"
        elif self.status == SyncStatus.SKIPPED:
            return f"SyncResult({self.sync_type.value}:{self.connection_name} SKIPPED: {self.message})"
        elif self.status == SyncStatus.NO_CHANGES:
            return f"SyncResult({self.sync_type.value}:{self.connection_name} NO_CHANGES)"
        return f"SyncResult({self.sync_type.value}:{self.connection_name} ERROR: {self.error})"


class FetchResult:
    """Result of fetching data from a source."""

    def __init__(
        self,
        available: bool,
        data: list[dict] | None = None,
        content_hash: str | None = None,
        skip_reason: str | None = None,
    ):
        self.available = available
        self.data = data or []
        self.content_hash = content_hash or ""
        self.skip_reason = skip_reason


class BaseSyncHandler(ABC):
    """Base class for sync handlers."""

    @property
    @abstractmethod
    def sync_type(self) -> SyncType:
        pass

    @abstractmethod
    async def fetch_current_data(self, conn: Connection) -> FetchResult:
        pass

    @abstractmethod
    async def get_cached_hash(self, conn: Connection) -> str | None:
        pass

    @abstractmethod
    async def apply_changes(
        self,
        conn: Connection,
        new_data: list[dict],
        is_full_refresh: bool,
    ) -> tuple[int, int, int]:
        pass

    @abstractmethod
    async def update_connection_hash(self, conn: Connection, content_hash: str) -> None:
        pass

    @abstractmethod
    async def update_connection_status(
        self,
        conn: Connection,
        status: str,
        message: str | None = None
    ) -> None:
        pass


async def run_in_executor(executor: ThreadPoolExecutor, func: Callable, *args, **kwargs) -> Any:
    """Run a blocking function in a thread pool executor."""
    loop = asyncio.get_event_loop()
    if kwargs:
        func = partial(func, **kwargs)
    return await loop.run_in_executor(executor, func, *args)


class SyncManager:
    """Manages background sync tasks for all connections."""

    def __init__(self):
        self._handlers: dict[SyncType, BaseSyncHandler] = {}
        self._running = False
        self._tasks: list[asyncio.Task] = []
        self._sync_in_progress: dict[tuple[int, SyncType], bool] = {}
        self._executor = ThreadPoolExecutor(max_workers=4, thread_name_prefix="sync_")

    def register_handler(self, handler: BaseSyncHandler) -> None:
        self._handlers[handler.sync_type] = handler
        logger.info(f"Registered sync handler for {handler.sync_type.value}")

    def is_sync_running(self, connection_id: int, sync_type: SyncType) -> bool:
        return self._sync_in_progress.get((connection_id, sync_type), False)

    async def sync_connection(
        self,
        conn: Connection,
        sync_type: SyncType,
        force_full: bool = False,
    ) -> SyncResult:
        """Sync a single connection."""
        handler = self._handlers.get(sync_type)
        if not handler:
            return SyncResult(
                status=SyncStatus.ERROR,
                sync_type=sync_type,
                connection_name=conn.name,
                error=f"No handler registered for {sync_type.value}",
            )

        key = (conn.id, sync_type)
        if self._sync_in_progress.get(key, False):
            return SyncResult(
                status=SyncStatus.SKIPPED,
                sync_type=sync_type,
                connection_name=conn.name,
                message="Sync already in progress",
            )

        self._sync_in_progress[key] = True
        start_time = datetime.utcnow()

        try:
            fetch_result = await handler.fetch_current_data(conn)

            if not fetch_result.available:
                logger.info(
                    f"Sync: {conn.name}/{sync_type.value} not connected: "
                    f"{fetch_result.skip_reason}"
                )
                await handler.update_connection_status(
                    conn, status="not_connected", message=fetch_result.skip_reason
                )
                return SyncResult(
                    status=SyncStatus.NOT_CONNECTED,
                    sync_type=sync_type,
                    connection_name=conn.name,
                    message=fetch_result.skip_reason,
                    duration_seconds=(datetime.utcnow() - start_time).total_seconds(),
                )

            new_data = fetch_result.data
            new_hash = fetch_result.content_hash

            if not new_data and fetch_result.available:
                duration = (datetime.utcnow() - start_time).total_seconds()
                logger.info(f"Sync: {conn.name}/{sync_type.value} connected (no bulk data)")
                await handler.update_connection_status(conn, status="connected", message=None)
                return SyncResult(
                    status=SyncStatus.CONNECTED,
                    sync_type=sync_type,
                    connection_name=conn.name,
                    duration_seconds=duration,
                )

            cached_hash = await handler.get_cached_hash(conn)

            if not force_full and cached_hash and cached_hash == new_hash:
                logger.debug(f"No changes detected for {conn.name}/{sync_type.value}")
                return SyncResult(
                    status=SyncStatus.NO_CHANGES,
                    sync_type=sync_type,
                    connection_name=conn.name,
                    items_fetched=len(new_data),
                    is_incremental=True,
                    content_hash=new_hash,
                    duration_seconds=(datetime.utcnow() - start_time).total_seconds(),
                )

            is_incremental = cached_hash is not None and not force_full

            added, updated, removed = await handler.apply_changes(
                conn, new_data, is_full_refresh=force_full or cached_hash is None,
            )

            await handler.update_connection_hash(conn, new_hash)

            duration = (datetime.utcnow() - start_time).total_seconds()
            logger.info(
                f"Sync completed for {conn.name}/{sync_type.value}: "
                f"+{added} ~{updated} -{removed} in {duration:.1f}s "
                f"({'incremental' if is_incremental else 'full'})"
            )

            return SyncResult(
                status=SyncStatus.SUCCESS,
                sync_type=sync_type,
                connection_name=conn.name,
                items_fetched=len(new_data),
                items_added=added,
                items_updated=updated,
                items_removed=removed,
                is_incremental=is_incremental,
                content_hash=new_hash,
                duration_seconds=duration,
            )

        except Exception as e:
            logger.exception(f"Sync failed for {conn.name}/{sync_type.value}")
            try:
                await handler.update_connection_status(conn, status="error", message=str(e))
            except Exception:
                pass
            return SyncResult(
                status=SyncStatus.ERROR,
                sync_type=sync_type,
                connection_name=conn.name,
                error=str(e),
                duration_seconds=(datetime.utcnow() - start_time).total_seconds(),
            )
        finally:
            self._sync_in_progress[key] = False

    async def _startup_sync(self) -> None:
        """Run initial sync for all enabled connections."""
        logger.info("Running startup sync for all enabled connections...")

        with get_session_context() as session:
            stmt = select(Connection).where(
                Connection.is_active == True,
                Connection.sync_enabled == True,
            )
            connections = list(session.exec(stmt).all())

        if not connections:
            logger.info("No active connections with sync enabled")
            return

        semaphore = asyncio.Semaphore(3)

        async def sync_with_limit(conn: Connection) -> None:
            async with semaphore:
                for sync_type in self._handlers.keys():
                    await self.sync_connection(conn, sync_type)

        tasks = [sync_with_limit(c) for c in connections]
        await asyncio.gather(*tasks, return_exceptions=True)
        logger.info(f"Startup sync completed for {len(connections)} connections")

    async def _polling_loop(self) -> None:
        """Main polling loop."""
        poll_interval = getattr(settings, 'SYNC_POLL_INTERVAL_SECONDS', 60)

        while self._running:
            try:
                with get_session_context() as session:
                    stmt = select(Connection).where(
                        Connection.is_active == True,
                        Connection.sync_enabled == True,
                    )
                    connections = list(session.exec(stmt).all())

                now = datetime.utcnow()

                for conn in connections:
                    if conn.last_sync_at is None:
                        needs_sync = True
                    else:
                        last = conn.last_sync_at
                        if last.tzinfo is not None:
                            last = last.replace(tzinfo=None)
                        next_sync = last + timedelta(minutes=conn.sync_interval_minutes)
                        needs_sync = now >= next_sync

                    if needs_sync:
                        for sync_type in self._handlers.keys():
                            if not self.is_sync_running(conn.id, sync_type):
                                asyncio.create_task(
                                    self.sync_connection(conn, sync_type)
                                )

            except Exception as e:
                logger.exception("Error in sync polling loop")

            await asyncio.sleep(poll_interval)

    async def start(self) -> None:
        if self._running:
            logger.warning("Sync manager already running")
            return

        self._running = True
        logger.info("Starting sync manager...")

        startup_task = asyncio.create_task(self._startup_sync())
        self._tasks.append(startup_task)

        poll_task = asyncio.create_task(self._polling_loop())
        self._tasks.append(poll_task)

        logger.info("Sync manager started (startup sync running in background)")

    async def stop(self) -> None:
        if not self._running:
            return

        logger.info("Stopping sync manager...")
        self._running = False

        for task in self._tasks:
            task.cancel()
            try:
                await task
            except asyncio.CancelledError:
                pass

        self._tasks.clear()
        self._executor.shutdown(wait=False)
        logger.info("Sync manager stopped")


# Global instance
_sync_manager: SyncManager | None = None


def get_sync_manager() -> SyncManager:
    global _sync_manager
    if _sync_manager is None:
        _sync_manager = SyncManager()
    return _sync_manager


async def start_sync_manager() -> None:
    from app.services.symbol_sync_handler import SymbolSyncHandler

    manager = get_sync_manager()
    manager.register_handler(SymbolSyncHandler())
    await manager.start()


async def stop_sync_manager() -> None:
    manager = get_sync_manager()
    await manager.stop()
