...
from app.services.live_summary_service import (
    ...
    derive_sync_state as _derive_sync_state,
    load_container_info_for_live as _load_container_info_for_live,
    serialize_live_summary_from_payload as _serialize_live_summary_from_payload,
)
...
@router.get(
    "/instances",
    response_model=list[LiveStrategySummaryRead],
    deprecated=True,
    description=(
        "Deprecated: GET /strategies/ now embeds a `live_summary` per strategy. "
        "This endpoint is kept for the strategy-detail history tab "
        "(filter by `strategy_id` with `include_stopped=true`)."
    ),
)
